...
for k in range(0, len(fold)):
    if fold[k][0] == 'y':
        for i in range(fold[k][1] + 1, len(grid)):
            for j in range(0, len(grid[i])):
                if grid[i][j] == '#':
                    grid[fold[k][1] - (i - fold[k][1])][j] = '#'
        for i in range(len(grid) - 1, fold[k][1] - 1, -1):
            del grid[i]
    else:
        for i in range(0, len(grid)):
            for j in range(fold[k][1] + 1, len(grid[i])):
                if grid[i][j] == '#':
                    grid[i][fold[k][1] - (j - fold[k][1])] = '#'
        for i in range(0, len(grid)):
            for j in range(len(grid[i]) - 1, fold[k][1] - 1, -1):
                del grid[i][j]

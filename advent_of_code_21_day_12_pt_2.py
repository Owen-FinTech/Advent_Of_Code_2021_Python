...
options = [['start', lower_visited.copy(), 'no']]
options[0][1]['start'] = 2
    ...
    for i in range(0, len(options)):
        for j in range(0, len(adj[options[i][0]])):
            if adj[options[i][0]][j] in lower_keys:
                if options[i][1][adj[options[i][0]][j]] < 2 and options[i][2] == 'no':
                    new = copy.deepcopy(options[i])
                    new[0] = adj[options[i][0]][j]
                    new[1][adj[options[i][0]][j]] += 1
                    if new[1][adj[options[i][0]][j]] == 2:
                        new[2] = 'yes'
                    temp.append(new)
                elif options[i][1][adj[options[i][0]][j]] < 1 and options[i][2] == 'yes':
                    new = copy.deepcopy(options[i])
                    new[0] = adj[options[i][0]][j]
                    new[1][adj[options[i][0]][j]] += 1
                    temp.append(new)
